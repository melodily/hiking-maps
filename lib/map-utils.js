...
// ── Overpass fetch with retry + mirror rotation ────────────────────────────
...
const OVERPASS_DEADLINE_MS = 30_000;
const OVERPASS_BACKOFF_BASE_MS = 2_000;

export async function fetchOverpass(query, { deadline = OVERPASS_DEADLINE_MS } = {}) {
  ...
  while (true) {
    for (const server of OVERPASS_SERVERS) {
      ...
      if (elapsed >= deadline) {
        throw new Error('Overpass unavailable: timed out after ' + deadline + 'ms');
      }
      ...
      if (resp.status !== 504) throw new Error(`Overpass HTTP ${resp.status}`);
      // 504 — try next server
    }

    // Completed a full round with no success — backoff before next round
    round++;
    const backoff = OVERPASS_BACKOFF_BASE_MS * round;
